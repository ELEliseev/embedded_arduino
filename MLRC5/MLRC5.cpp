...
MLRC5::MLRC5(bool TxRx, unsigned char Writepin)
{
    if(TxRx){
...
void MLRC5::decodePulse(unsigned char signal, unsigned long period)
{
...
void MLRC5::decodeEvent(unsigned char event)
{
...
bool MLRC5::read(unsigned int *message)
{
    /* Note that the input value read is inverted from the theoretical signal,
       ...
       So when the value changes, the inverted value that we get from reading the Readpin
...
void MLRC5::emit(bool bit){// отправка 1 или 0 
if(bit){
...
bool MLRC5::write( unsigned int WriteValue)
{
    
   Serial.println("Start");
    emit(true);//отправляем две 1 старт
    emit(true);
  for(int i=11; i>=0; i--){//отправляем посылку старш байтом вперед
      Serial.print((WriteValue >> i) & 1);
    if(((WriteValue >> i) & 1)==1){
      emit(true);
    }
    if(((WriteValue >> i) & 1)==0){
...
bool MLRC5::read(unsigned char *toggle, unsigned char *address, unsigned char *command)
{
